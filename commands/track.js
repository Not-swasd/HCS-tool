...
module.exports = {
	...
	async execute(interaction, client) {
		try {
			...
			await interaction.reply({ embeds: [new MessageEmbed().setTitle("🔍 검색 중... (많으면 약 1분정도 소요)").setColor("BLUE")], ephemeral: true });
			...
			if (!school) return interaction.editReply({ embeds: [new MessageEmbed().setTitle("❌ 정보를 다시 확인해 주세요!").setColor("RED")], ephemeral: true });
			await interaction.editReply({
				embeds: [new MessageEmbed().setColor("GREEN").setTitle("✅ 트래킹 성공").setDescription(`**\`${school.orgName}(${r[school.scCode]})\`**에서 **\`${name}\`**님의 정보를 찾았습니다! (걸린 시간: ${((Date.now() - startedTime) / 1000) + 1}초)`)]
			});
		} catch (e) {
			using.remove(interaction.user.id);
