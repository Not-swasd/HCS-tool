...
module.exports = {
	...
	async execute(interaction, client) {
		try {
			if (using.includes(interaction.user.id)) return interaction.reply({ embeds: [new MessageEmbed().setTitle("❌ 기다리쇼").setColor("RED")] });
			...
			await interaction.reply({ embeds: [new MessageEmbed().setTitle("🔍 검색 중... (최대 약 1분 소요)").setColor("BLUE")], ephemeral: true });
